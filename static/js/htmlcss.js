/* ══════════════════════════════════════════════════════════
   COGNITIO · htmlcss.js  —  Curso de HTML & CSS para Iniciantes
   Mesmo padrão do javascript.js (topbar, sidebar, progresso,
   toast) + laboratório de preview ao vivo (iframe sandbox).
   ══════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  /* ─── CONFIG ─── */
  const COURSE_TITLE  = 'HTML & CSS';
  const TOTAL_MODULES = 16;
  const STORAGE_KEY   = 'htmlcss_done';

  const MODULES = [
    { num: 1,  title: 'Introdução ao HTML',                    route: '/html-css'           },
    { num: 2,  title: 'Texto, Listas, Links e Imagens',         route: '/html-css/modulo2'   },
    { num: 3,  title: 'Formulários e Tabelas',                  route: '/html-css/modulo3'   },
    { num: 4,  title: 'Introdução ao CSS',                      route: '/html-css/modulo4'   },
    { num: 5,  title: 'Flexbox e Grid',                         route: '/html-css/modulo5'   },
    { num: 6,  title: 'Responsividade e Boas Práticas',         route: '/html-css/modulo6'   },
    { num: 7,  title: 'HTML Semântico e Estrutura Acessível',   route: '/html-css/modulo7'   },
    { num: 8,  title: 'Flexbox Avançado',                       route: '/html-css/modulo8'   },
    { num: 9,  title: 'CSS Grid Avançado',                      route: '/html-css/modulo9'   },
    { num: 10, title: 'Variáveis CSS e Temas',                  route: '/html-css/modulo10'  },
    { num: 11, title: 'Animações e Transições',                 route: '/html-css/modulo11'  },
    { num: 12, title: 'Pseudo-classes e Pseudo-elementos',      route: '/html-css/modulo12'  },
    { num: 13, title: 'Responsivo e Mobile-First',              route: '/html-css/modulo13'  },
    { num: 14, title: 'Arquitetura CSS',                        route: '/html-css/modulo14'  },
    { num: 15, title: 'Acessibilidade Web',                     route: '/html-css/modulo15'  },
    { num: 16, title: 'Projeto Final',                          route: '/html-css/modulo16'  },
  ];
    ...
    bar.innerHTML = `
      ...
      <div class="mod-topbar-right">
        <div class="progress-pill">
          <i class="fa-brands fa-html5" style="font-size:.7rem"></i>
  ...
  function buildSidebar() {
    ...
    aside.innerHTML = `
      <span class="sidebar-label"><i class="fa-brands fa-html5" style="margin-right:5px"></i>Módulos do curso</span>
  ...
  function greetUser() {
    setTimeout(() => {
      if (current.num === 1) showToast('Bem-vindo ao curso de HTML & CSS! 🌐', 3500);
  ...
  function injectModTag() {
    ...
    tag.innerHTML = `<i class="fa-brands fa-html5"></i> Módulo ${current.num} de ${TOTAL_MODULES}`;
    ...
    document.querySelectorAll('a.btn-proximo').forEach(btn => {
      if (!btn.getAttribute('href') || btn.getAttribute('href') === '#') {
        ...
        btn.addEventListener('click', e => {
          ...
          showToast('Curso de HTML & CSS concluído! 🏆', 4000);
  ...
  /* ══════════════════════════════
     11. Laboratório HTML/CSS (preview ao vivo)
     Renderiza o código do editor dentro de um <iframe>
     usando srcdoc — 100% no navegador, sem risco algum,
     pois o iframe é sandboxed e isolado da página.
     ══════════════════════════════ */
  function initHtmlLab() {
    const editor  = document.getElementById('html-lab-editor');
    const preview = document.getElementById('html-lab-preview');
    const runBtn  = document.getElementById('html-lab-run');
    const resetBtn = document.getElementById('html-lab-reset');
    if (!editor || !preview || !runBtn) return;
    ...
    document.querySelectorAll('.html-lab-snippet').forEach(btn => {
      btn.addEventListener('click', () => {
        editor.value = btn.dataset.code || btn.textContent;
        run();
      });
    });
    ...
    // Renderiza o preview inicial assim que a página carrega
    run();
  }
