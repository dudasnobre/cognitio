/* ══════════════════════════════════════════════════════════
   COGNITIO · informatica.js  —  Curso de Informática Básica
   ══════════════════════════════════════════════════════════ */
...
(function () {
  ...
  /* ─── CONFIG ─── */
  const COURSE_TITLE  = 'Informática Básica';
  const TOTAL_MODULES = 10;

  const MODULES = [
    { num: 1,  title: 'Introdução & Hardware',        route: '/infor' },
    { num: 2,  title: 'Sistema Operacional',          route: '/modulo2'  },
    { num: 3,  title: 'Processador de Texto',         route: '/modulo3'  },
    { num: 4,  title: 'Planilhas com Excel',          route: '/modulo4'  },
    { num: 5,  title: 'Internet & Segurança',         route: '/modulo5'  },
    { num: 6,  title: 'E-mail & Comunicação',         route: '/modulo6'  },
    { num: 7,  title: 'Apresentações (PowerPoint)',   route: '/modulo7'  },
    { num: 8,  title: 'Nuvem e Produtividade',        route: '/modulo8'  },
    { num: 9,  title: 'Segurança Digital',            route: '/modulo9'  },
    { num: 10, title: 'Mercado de Trabalho Digital',  route: '/modulo10' },
  ];

  /* ─── Utils ─── */
  const currentRoute = () => window.location.pathname;
  const isDone   = n => JSON.parse(localStorage.getItem('cognitio_done') || '[]').includes(n);
  const markDone = n => {
    const done = JSON.parse(localStorage.getItem('cognitio_done') || '[]');
    if (!done.includes(n)) { done.push(n); localStorage.setItem('cognitio_done', JSON.stringify(done)); }
  };
  const getDoneCount = () => JSON.parse(localStorage.getItem('cognitio_done') || '[]').length;
    ...
    bar.innerHTML = `
      ...
      <div class="mod-topbar-right">
        <div class="progress-pill">
          <i class="fa-solid fa-fire" style="font-size:.7rem"></i>
  ...
  function greetUser() {
    setTimeout(() => {
      if (current.num === 1) {
        showToast('Bem-vindo ao Módulo 1 de Informática! 🚀', 3500);
      } else if (current.num === TOTAL_MODULES && isDone(TOTAL_MODULES - 1)) {
        showToast('Último módulo de Informática! Quase lá 🎉', 3500);
      } else if (isDone(current.num - 1)) {
        showToast(`Módulo ${current.num - 1} de Informática concluído! ✔`, 3000);
      }
    }, 900);
  }
    ...
    btns.forEach(btn => {
      if (!btn.getAttribute('href') || btn.getAttribute('href') === '#') {
        ...
        btn.addEventListener('click', e => {
          ...
          showToast('Parabéns! Curso concluído! 🏆', 4000);
  ...
  function injectModTag() {
    ...
    tag.innerHTML = `<i class="fa-solid fa-bookmark"></i> Módulo ${current.num} de ${TOTAL_MODULES}`;
